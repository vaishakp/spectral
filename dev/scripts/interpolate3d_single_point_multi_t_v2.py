...
# Load grid structure
#####################
dl = SXSDataLoader(run_dir=run_dir, 
                   subdomain="SphereC0", 
                   metric_dir='PsiKappa',)
...
if mpi_rank==0:
    ...
    message(f"Ang Coord values Theta{dl.theta_grid[index_x, index_y]} Phi{dl.phi_grid[index_x, index_y]}")
else:
    index_x = None
    index_y = None
...
one_set_of_coords = np.array([[r_grid[index_x, index_y],], [dl.theta_grid[index_x, index_y],], [dl.phi_grid[index_x, index_y],]])
...
# Initialize context
interp3d = Interpolate3D(sphp_output_grid=sphp_output_grid_list,
                        raw_data=input_data,
                        r_min=dl.r_min,
                        r_max=dl.r_max,
                        run_dir=run_dir,
...
if mpi_rank==0:
    message("SPHP output grid (after Interpolate3D) \t", interp3d.sphp_output_grid)
...
if mpi_rank==0:
    ...
    message(f"Mode shape {interp3d.interpolant.mode(2, 2, extra_indices=(0,)).shape}")
    message("Evaluating...")
...
if mpi_rank==0:
    ...
    for index in range(n_t_steps):
        ...
        np.testing.assert_array_almost_equal(interp3d.interpolated_data[index].real, 
                                         [input_data[index][0][index_x, index_y]], 
                                         7, 
